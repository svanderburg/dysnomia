...
makeTest {
  ...
  testScript =
    ''
      ...
      $machine->mustSucceed("postgresqlUsername=postgres dysnomia --type postgresql-database --operation activate --component ${postgresql_database} --environment");
      my $result = $machine->mustSucceed("echo 'select * from test' | su postgres -s /bin/sh -c 'psql --file - testdb'");
      ...
      $machine->mustSucceed("postgresqlUsername=postgres dysnomia --type postgresql-database --operation snapshot --component ${postgresql_database} --environment");
      ...
      # succeed.
      $machine->mustSucceed("echo \"insert into test values ('Bye world');\" | su -s /bin/sh postgres -c 'psql --file - testdb'");
      $machine->mustSucceed("postgresqlUsername=postgres dysnomia --type postgresql-database --operation snapshot --component ${postgresql_database} --environment");
      ...
      $machine->mustSucceed("postgresqlUsername=postgres dysnomia --type postgresql-database --operation collect-garbage --component ${postgresql_database} --environment");
      $machine->mustSucceed("echo 'select * from test;' | su postgres -s /bin/sh -c 'psql --file - testdb'");

      # Deactivate the PostgreSQL database again. This test should succeed.
      $machine->mustSucceed("postgresqlUsername=postgres dysnomia --type postgresql-database --operation deactivate --component ${postgresql_database} --environment");
      ...
      $machine->mustSucceed("postgresqlUsername=postgres dysnomia --type postgresql-database --operation collect-garbage --component ${postgresql_database} --environment");
      $machine->mustFail("echo 'select * from test;' | su postgres -s /bin/sh -c 'psql --file - testdb'");
      ...
      $machine->mustSucceed("postgresqlUsername=postgres dysnomia --type postgresql-database --operation restore --component ${postgresql_database} --environment");
